...
<div>
                    ...
                    <b-col class="p-0 m-0">
                        <ValidationObserver ref="form"  v-slot="{ handleSubmit, reset }">
                            <b-form  @submit.prevent="handleSubmit(searchData)" @reset.prevent="reset" >
                                <div class="row">
                                    <b-col md="4">
                                        <Select @return-value="setReturnData({ branch_id: $event})" :input="{ cols: 3, name: 'branch_id', rules: '', vmodel: search.branch_id, options: branchList }"/>
                                    </b-col>
                                    <b-col md="4">
  ...
  <transition name="bounce">
    <b-modal id="modal-1"
        ...
        <p class="vheader">Station Entry</p>
            <div>
                <Form :id='editId'/>
            </div>
    </b-modal>
...
export default {
    ...
    methods: {
		loadData () {
			...
			RestApi.getData(baseUrl, 'station/list', params).then(response => {
				if (response.success) {
        ...
        changeStatus (item) {
            this.toggleStatus('station/delete', item)
        }
	}
}
</script>