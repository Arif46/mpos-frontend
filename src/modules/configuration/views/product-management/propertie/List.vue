...
<div>
  ...
  <transition name="bounce">
    <b-modal id="modal-1"
        ...
        <p class="vheader">Propertie Entry</p>
            <div>
                <Form :id='editId'/>
            </div>
    </b-modal>
...
import { propertieList, propertieStatus } from '../../../api/routes'
...
export default {
    ...
    methods: {
		loadData () {
			...
			RestApi.getData(baseUrl, propertieList, params).then(response => {
				if (response.success) {
        ...
        changeStatus (item) {
            this.toggleStatus(propertieStatus, item)
        }
	}
}
</script>