                    ...
                    <ValidationObserver ref="form"  v-slot="{ handleSubmit, reset }">
                        <b-form  @submit.prevent="handleSubmit(formSubmit)" @reset.prevent="reset" >
                            <b-row>
                              ...
                              <b-col>
                                  <Input @return-value="setReturnData({ value: $event})" :input="{ cols: 12, type: 'text', name: 'Value', rules: 'required', vmodel: formData.value }"/>
                              </b-col>
                            </b-row>
...
import { propertieStore, propertieUpdate } from '../../../api/routes'
...
export default {
  ...
  methods: {
    async formSubmit () {
      const result = this.submitHandler(propertieStore, propertieUpdate, this.formData)
