...
<div>
  ...
  <transition name="bounce">
    <b-modal id="modal-1"
        ...
        <p class="vheader">Unit Name Entry</p>
            <div>
                <Form :id='editId'/>
            </div>
    </b-modal>
...
import { unitList, unitStatus } from '../../../api/routes'
...
export default {
    ...
    methods: {
		loadData () {
			...
			RestApi.getData(baseUrl, unitList, params).then(response => {
				if (response.success) {
        ...
        changeStatus (item) {
            this.toggleStatus(unitStatus, item)
        }
	}
}
</script>