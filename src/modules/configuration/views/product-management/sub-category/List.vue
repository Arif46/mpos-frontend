...
<div>
  ...
  <transition name="bounce">
    <b-modal id="modal-1"
        ...
        <p class="vheader">Sub Category Entry</p>
            <div>
                <Form :id='editId'/>
            </div>
    </b-modal>
...
import { subCategoryList, subCategoryStatus } from '../../../api/routes'
...
export default {
    ...
    methods: {
		loadData () {
			...
			RestApi.getData(baseUrl, subCategoryList, params).then(response => {
				if (response.success) {
        ...
        formatData (data) {
            return data.map(item => {
                const category = this.getStoreData('categoryList', item.category_id)
                return Object.assign(item, { category_name: category })
            })
        },
        changeStatus (item) {
            this.toggleStatus(subCategoryStatus, item)
        }
	}
}
</script>