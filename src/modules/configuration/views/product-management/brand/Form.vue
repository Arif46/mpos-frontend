                ...
                <b-overlay :show="false">
                    <ValidationObserver ref="form"  v-slot="{ handleSubmit, reset }">
                        <b-form  @submit.prevent="handleSubmit(formSubmit)" @reset.prevent="reset" >
                            <b-row>
                              <b-col>
                                  <Input @return-value="setReturnData({ name: $event})" :input="{ cols: 12, type: 'text', name: 'Brand Name', rules: 'required', vmodel: formData.name }"/>
                              </b-col>
                            </b-row>
...
import { brandStore, brandUpdate } from '../../../api/routes'
...
export default {
  ...
  methods: {
    async formSubmit () {
      const result = this.submitHandler(brandStore, brandUpdate, this.formData)
