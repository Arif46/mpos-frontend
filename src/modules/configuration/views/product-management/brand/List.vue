...
import { brandList, brandStatus } from '../../../api/routes'
...
export default {
    ...
    methods: {
		loadData () {
			...
			RestApi.getData(baseUrl, brandList, params).then(response => {
				if (response.success) {
        ...
        changeStatus (item) {
            this.toggleStatus(brandStatus, item)
        }
	}
}
</script>