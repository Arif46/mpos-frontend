...
<div>
  ...
  <transition name="bounce">
    <b-modal id="modal-1"
        ...
        <p class="vheader">Category Entry</p>
            <div>
                <Form :id='editId'/>
            </div>
    </b-modal>
...
import { categoryList, categoryStatus } from '../../../api/routes'
...
export default {
    ...
    methods: {
		loadData () {
			...
			RestApi.getData(baseUrl, categoryList, params).then(response => {
				if (response.success) {
        ...
        changeStatus (item) {
            this.toggleStatus(categoryStatus, item)
        }
	}
}
</script>