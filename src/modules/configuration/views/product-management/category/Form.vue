                ...
                <b-overlay :show="false">
                    <ValidationObserver ref="form"  v-slot="{ handleSubmit, reset }">
                        <b-form  @submit.prevent="handleSubmit(formSubmit)" @reset.prevent="reset" >
                            <b-row>
                              <b-col>
                                  <Input @return-value="setReturnData({ name: $event})" :input="{ cols: 12, type: 'text', name: 'Category', rules: 'required', vmodel: formData.name }"/>
                              </b-col>
                            </b-row>
...
import { categoryStore, categoryUpdate } from '../../../api/routes'
...
export default {
  ...
  methods: {
    async formSubmit () {
      const result = this.submitHandler(categoryStore, categoryUpdate, this.formData)
