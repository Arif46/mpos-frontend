...
<div>
  ...
  <transition name="bounce">
    <b-modal id="modal-1"
        ...
        <p class="vheader">Customer Entry</p>
            <div>
                <Form :id='editId'/>
            </div>
    </b-modal>
...
import { customerList, customerStatus } from '../../../api/routes'
...
export default {
    ...
    computed: {
        fields () {
            const labels = [
                ...
                { label: 'Customer Type Name', class: 'text-center' },
    ...
    methods: {
		loadData () {
			...
			RestApi.getData(baseUrl, customerList, params).then(response => {
				if (response.success) {
        ...
        formatData (data) {
            return data.map(item => {
                const customerTypeObj = this.getStoreData('customerTypeList', item.customer_type_id)
                return Object.assign(item, { customer_type: customerTypeObj })
            })
        },
        changeStatus (item) {
            this.toggleStatus(customerStatus, item)
        }
	}
}
</script>