...
<div>
  ...
  <transition name="bounce">
    <b-modal id="modal-1"
        ...
        <p class="vheader">Supplier Entry</p>
            <div>
                <Form :id='editId'/>
            </div>
    </b-modal>
...
import { supplierList, supplierStatus } from '../../../api/routes'
...
export default {
    ...
    computed: {
        fields () {
            const labels = [
                ...
                { label: 'Mobile No', class: 'text-center' },
                { label: 'Bank Name', class: 'text-center' },
    ...
    methods: {
		loadData () {
			...
			RestApi.getData(baseUrl, supplierList, params).then(response => {
				if (response.success) {
        ...
        changeStatus (item) {
            this.toggleStatus(supplierStatus, item)
        }
	}
}
</script>