			...
			<b-col lg="10" offset-lg="1" sm="12">
				<b-overlay :show="loading">
					<ValidationObserver ref="form"  v-slot="{ handleSubmit, reset }">
						...
						<ValidationProvider name="Sub Category" vid="sub_content_id" rules="required">
							<b-form-group
							class="row"
							label-cols-sm="12"
							label-for="sub_content_id"
							slot-scope="{ valid, errors }"
							>
							<template v-slot:label>
							Sub Content <span class="text-danger">*</span>
							</template>
							<b-form-select
							v-model="formData.sub_content_id"
							:state="errors[0] ? false : (valid ? true : null)"
							:options="subContentList">
							</b-form-select>
							<div class="invalid-feedback">
								{{ errors[0] }}
							</div>
...
export default {
  ...
  watch: {
	  ...
	  'formData.content_id': function (newVal, oldVal) {
		  if (newVal !== oldVal) {
			  this.subContentList = this.getSubContentList(newVal)
		  }
	  }
  },
  methods: {
    ...
    async register () {
        ...
        let result = null
        if (this.id) {
            result = await RestApi.putData(baseUrl, `${'api/sub-sub-content/update'}/${this.id}`, this.formData)
        } else {
	...
	getSubContentList (Id) {
		return this.$store.state.commonObj.subContentList.filter(item => item.content_id === Id)
	}
  }
}
</script>
